...
class EventsController @Inject()(
  ...
  def saveDeclarationEvent(eori: String, lrn: String): Action[JsValue] = Action.async(parse.json) { implicit request =>
    authAction.withAuthAndEnrolmentCheck[JsValue](eori){ _ =>
      request.body.validate[SaveDeclarationEventRequest].fold(_ => {
